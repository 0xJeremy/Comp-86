...
public class frame extends JFrame
{
	...
	public frame(String window_name, int height, int width)
	{
		...
		data.add(score);
		JLabel enemies = new JLabel("Enemies = " + canvas.get_enemies());
		data.add(enemies);
		add(data, BorderLayout.LINE_END);
		...
		ActionButton button3 = new ActionButton("Toggle Zoom");
		button3.add_single_print("Zoom Toggled");
		...
		fire_button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				score_counter++;
				num_enemies = canvas.get_enemies();
				update_scoreboard(score, "Score = " + score_counter);
				update_scoreboard(enemies, "Enemies = " + Integer.toString(num_enemies));
			}
		});
	...
	public void update_scoreboard(JLabel label, String text)
	{
		label.setText(text);
	}
